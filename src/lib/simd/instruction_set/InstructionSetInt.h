...
namespace math
{
	...
	private:
		...
		{
			float breakpoint = 0;
			// AVX2 does not support direct multiplication of 32-bit integers. You may need to use a workaround or fallback to SSE4.1
		}

		template<Options Option>
		static void mul_avx(int* result, const int* a, const int* b, size_t size)
		{
			// AVX does not support direct multiplication of 32-bit integers. You may need to use a workaround or fallback to SSE4.1
		}
		...
		{
			// SSSE3 does not support direct multiplication of 32-bit integers. You may need to use a workaround or fallback to SSE4.1
		}

		template<Options Option>
		static void mul_sse3(int* result, const int* a, const int* b, size_t size)
		{
			// SSE3 does not support direct multiplication of 32-bit integers. You may need to use a workaround or fallback to SSE4.1
		}

		template<Options Option>
		static void mul_fallback(int* result, const int* a, const int* b, size_t size, size_t dim)
		{
